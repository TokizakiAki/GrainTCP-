...
export default { fetch: (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new Response('Error 404') }; 
...
const raceSprout = (h, p) => { if (CFG.concur <= 1) return sprout(h, p); const ts = Array(CFG.concur).fill().map(() => sprout(h, p)); return Promise.any(ts).then(w => { ts.forEach(t => t.then(s => s !== w && s.close(), () => {})); return w; }); };
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    ...
    if (!sock) { 
        ...
        sock = await (g && l.length ? 
            (k => raceSprout(k.h, k.p || port))(pick()) : 
            raceSprout(host, port).catch(async () => { 
                const k = pick(); 
                if (!k) throw new Error('direct failed'); 
                return raceSprout(k.h, k.p || port); 
            })
        );
