...
const x = c => { if (c.length < 24 || !matchID(c)) return null; let o = 19 + c[17]; const p = (c[o] << 8) | c[o + 1]; let t = c[o + 2]; if (t !== 1) t += 1; const a = parseAddr(c, o + 3, t); return a ? { addrType: t, ...a, port: p } : null; };
...
const mkQ = (cap, qCap = cap, itemsMax = Math.max(1, qCap >> 8)) => {
  ...
  return { get bytes() { return qB; }, get size() { return q.length - h; }, get empty() { return h >= q.length; }, clear() { q = []; h = 0; qB = 0; },
    ...
    bundle(d) {
      ...
      let n = d.byteLength, e = h; while (e < q.length) { const qx = q[e], nn = n + qx.byteLength; if (nn > cap) break; n = nn; e++; }
      if (e === h) return [d, 0]; const out = buf ||= new Uint8Array(cap); out.set(d);
      for (let o = d.byteLength; h < e;) { const qx = q[h]; q[h++] = undefined; qB -= qx.byteLength; out.set(qx, o); o += qx.byteLength; } trim(); return [out.subarray(0, n), 1]; } }; };
const mkDn = w => {
...
const mill = async (rd, w) => { const reader = rd.getReader({ mode: 'byob' }), tx = mkDn(w); let buf = new ArrayBuffer(CFG.chunk);
  try { for (;;) { const { done, value: v } = await reader.read(new Uint8Array(buf, 0, CFG.chunk)); if (done) break; if (!v?.byteLength) continue; if (v.byteLength >= (CFG.chunk >> 1)) tx.reap(), w.send(v), buf = new ArrayBuffer(CFG.chunk); else tx.send(v.slice()), buf = v.buffer; } tx.reap(); } catch {} finally { try { tx.reap(); } catch {} try { reader.releaseLock(); } catch {} } };
const ws = async (req, env) => {
  ...
  const _pathRaw = _url.pathname.match(/\/r=([^/?#]+)/i)?.[1] || ''; 
  const g = /&g/i.test(_pathRaw); 
  const l = (_pathRaw.replace(/&g/i, '') || (env?.R || '')).trim().split(/[\n,]+/).map(s => s.trim()).filter(Boolean);
  const pick = () => { 
    if (!l.length) return null; 
    const raw = l[Math.floor(Math.random() * l.length)]; 
    if (raw.startsWith('[')) { 
      const e = raw.indexOf(']'); 
      if (e > 0) { 
        const h = raw.slice(1, e), ps = raw.slice(e + 1), pt = ps.startsWith(':') ? parseInt(ps.slice(1)) : NaN; 
        return { h, p: pt > 0 && pt < 65536 ? pt : null }; 
      } 
    } 
    const i = raw.lastIndexOf(':'); 
    if (i > 0) { 
      const pt = parseInt(raw.slice(i + 1)); 
      if (pt > 0 && pt < 65536) return { h: raw.slice(0, i), p: pt }; 
    } 
    return { h: raw, p: null }; 
  };
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    ...
    if (!sock) { 
        ...
        const res = x(d); if (!res) throw wither(); 
        server.send(new Uint8Array([d[0], 0])); 
        const host = addr(res.addrType, res.targetAddrBytes), port = res.port, payload = d.subarray(res.dataOffset);
        sock = await (g && l.length ? 
            (k => raceSprout(fetcher, k.h, k.p || port))(pick()) : 
            raceSprout(fetcher, host, port).catch(async () => { 
                const k = pick(); 
                if (!k) throw new Error('direct failed'); 
                return raceSprout(fetcher, k.h, k.p || port); 
            })
        );
