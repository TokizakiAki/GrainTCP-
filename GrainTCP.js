...
const ws = async (req, env) => {
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    if (closed) break; if (!sock) { const [d] = uq.bundle(); if (!d) break; const res = x(d); if (!res) throw wither(); server.send(new Uint8Array([d[0], 0])); const host = addr(res.addrType, res.targetAddrBytes), port = res.port, payload = d.subarray(res.dataOffset); 
    sock = await raceSprout(fetcher, host, port).catch(() => {
        const i = r.lastIndexOf(':'); 
        return raceSprout(fetcher, i > 0 ? r.slice(0, i) : r, i > 0 ? parseInt(r.slice(i + 1)) : port);
    });
